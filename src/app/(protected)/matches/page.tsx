...
<main className="min-h-[calc(100vh-125px)] p-4 sm:p-6 bg-gradient-to-br from-background via-background to-primary/5">
  <div className="max-w-[1600px] mx-auto flex flex-col  lg:flex-row gap-0 lg:gap-6">
    ...
    <div className="max-w-2xl mx-auto">
      ...
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-3xl font-bold bg-gradient-to-r from-rose-500 via-rose-500 to-red-500 bg-clip-text text-transparent">
          Matches
        </h1>
      ...
      <div className="space-y-4">
        {matches.map((match) => (
          ...
          >
            ...
            <Link href={`/profile/${match.id}`} className="flex items-center gap-3 flex-1">
              ...
              <div className="flex flex-col min-w-0">
                <div className="flex items-center gap-2">
                  <h2 className="text-lg sm:text-xl font-semibold truncate">{match.name}</h2>
                  {match.age && <span className="text-muted-foreground">{match.age}</span>}
                </div>
                ...
                <p className="text-sm text-muted-foreground truncate max-w-[170px] sm:max-w-[240px]">
                  {match.isOnline 
                    ? "Active now 🔥" 
                    : getLastSeenText(match.lastActiveAt) || "Recently active"}
                </p>
              </div>
            </Link>
