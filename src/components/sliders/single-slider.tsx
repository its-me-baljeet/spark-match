...
export function SingleSlider({ min, max, value, onChange, label, unit, step = 1 }: SingleSliderProps) {
  const percent = ((value - min) / (max - min)) * 100;

  return (
    <div className="space-y-3">
      <label className="text-sm font-semibold text-foreground">{label}: {value} {unit}</label>
